...
Remaining for the income engine: wire + persist rental (mirroring employment 1b/1c), edit saved calcs (+ optional dedupe vs streams), build non-taxable / Social Security and self-employment (Form 1084), wire extractors to populate the input models, and finally retire `app/services/income_service.py` in favor of `app/income/`.
