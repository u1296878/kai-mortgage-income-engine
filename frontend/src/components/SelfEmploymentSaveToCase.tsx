...
import { saveSelfEmploymentCalculation } from "../api/income";
import {
...
export function SelfEmploymentSaveToCase({ caseId, form }: Props): JSX.Element {
  const [label, setLabel] = useState("");
  const navigate = useNavigate();
  const mutation = useMutation({
    mutationFn: () =>
      saveSelfEmploymentCalculation(caseId, {
        ...toSelfEmploymentPayload(form),
