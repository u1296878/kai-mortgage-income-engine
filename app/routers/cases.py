...
def create_case(
    ...
    return case_service.create_case(db, case.title, case.broker_id)
...
def list_cases(
    ...
    return case_service.list_cases(db, broker_id)
...
def get_case_with_documents(
    ...
    try:
        return case_service.get_case_with_documents(db, case_id, broker_id)
    except CaseNotFound as error:
        raise HTTPException(status_code=404, detail=str(error)) from error
