...
def test_vision_schedule_e_emits_two_flat_property_field_sets():
    ...
    assert by_name["schedule_e_property_a_address"].raw_text == "131 E 500 S Provo UT 84606"
...
def test_vision_schedule_e_converts_model_source_box_to_pdf_points():
    fields = _extract([_property("A", "131 E 500 S Provo UT 84606", 22480, 19943, 366)])

    rents = next(field for field in fields if field.field == "schedule_e_property_a_gross_rents")
    assert rents.page == 2
    assert rents.bounding_box.model_dump() == {"x1": 126.0, "y1": 158.4, "x2": 252.0, "y2": 198.0}
...
def _property(column: str, address: str, rents, expenses, days: int) -> dict:
    return {
        "column": column,
        "address": _entry(address, source_text=address),
        "fair_rental_days": _entry(days),
        "rents_received": _entry(rents),
        "insurance": _entry(0),
        "mortgage_interest": _entry(0),
        "other_interest": _entry(0),
        "taxes": _entry(0),
        "depreciation_depletion": _entry(0),
        "total_expenses": _entry(expenses),
    }


def _entry(value, source_text: str | None = None) -> dict:
    return {
        "value": value,
        "confidence": 0.9,
        "source_text": source_text or (str(value) if value is not None else None),
        "box": [0.2, 0.2, 0.4, 0.25] if value is not None else None,
        "page_index": 0 if value is not None else None,
    }
